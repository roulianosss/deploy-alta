...
`,a.map(r=>f.innerHTML+=`
  ...
  `),document.querySelectorAll(".support-card-container").forEach(v=>v.addEventListener("click",()=>a(v)))}}if(document.querySelector(".alta-news-talking-about-container")){let m=function(t){n.forEach(c=>{c.getAttribute("data-attr")===t&&c.classList.add("activePage")}),i.innerHTML="",o.slice((t-1)*4,t*4).forEach(c=>i.innerHTML+=`
    ...
    >
      ...
      >
       ${s.description}
        ...
        `})}});if(document.querySelector(".capital-calculator-section")){let t=function(){let _=l.valueAsNumber;const f=i.valueAsNumber,S=m.valueAsNumber/100,s=n.valueAsNumber;for(let p=0;p<f;p++)for(let h=0;h<12;h++){const y=S*_;_+=y*(1/12)+s}_=Math.round(_/1e3)*1e3,_.toString().split("").splice(-3,0," "),c(o,_,500)},c=function(_,f,S){const p=S/10;let h=0;const y=(f-0)/p;function L(){h+=y,h>=f?_.textContent=`${g(f)} €`:(_.textContent=`${g(h.toFixed(0))} €`,setTimeout(L,10))}L()},g=function(_){return _.toString().replace(/\B(?=(\d{3})+(?!\d))/g," ")};var calculateResult=t,animateNumber=c,numberWithSpaces=g;const o=document.querySelector(".capital-calculator-section__result-value"),e=document.querySelectorAll(".capital-calculator-section__cursors-container__cursor-container input"),l=document.querySelector("#first-range"),n=document.querySelector("#second-range"),i=document.querySelector("#third-range"),m=document.querySelector("#fourth-range");e.forEach(_=>_.addEventListener("input",function(){const f=(this.value-this.min)/(this.max-this.min)*100,S=_.value.split("");S.splice(-3,0," "),(_.name="first-range")&&(_.labels[0].textContent=`${S.join("")} ${_.getAttribute("data-attr")}`),this.style.background=`linear-gradient(to right, #ff6442 0%, #ff6442 ${f}%, #fbe1da ${f}%, #fbe1da 100%)`,t()})),t()}const T=document.querySelectorAll(".promo-banner-section");T.forEach(o=>{o.querySelector(".promo-banner-section__items-section__items");const e=o.querySelectorAll(".promo-banner-section__items-section__items__item"),l=o.querySelector(".promo-banner-section__items-section__left-arrow"),n=o.querySelector(".promo-banner-section__items-section__right-arrow");let i=0;e.length>1&&(n.style.display="block",l.style.display="block",n.addEventListener("click",()=>{i=(i+1)%e.length,m(i)},{passive:!0}),l.addEventListener("click",()=>{i=(i-1+e.length)%e.length,m(i)},{passive:!0}));function m(t){e.forEach(c=>{c.style.transform=`translateX(-${t*100}%)`})}m(i)});
